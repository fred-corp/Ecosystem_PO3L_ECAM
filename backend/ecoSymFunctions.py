...
def process(ecoSymDict, object, ecosystem, grid):
    if isinstance(object, Animal):
        ...
        if object.age > object.lifespan:
            ecosystem.add_object(Meat(object.UUID, "meat", object.x, object.y, 10, 0, ecoSymDict["meatCompostAfter"]))
...
def seek(zone, object, ecosystem):
    ...
    for coord in zone:
        ...
        if object.age >= object.adultAt:
            if (type(object) == type(find_object)) and (object.lifeform == find_object.lifeform) and (find_object.age >= object.adultAt) and (object.gender != find_object.gender) and (object.isPregnant == 0):
